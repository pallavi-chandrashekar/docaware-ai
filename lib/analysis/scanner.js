...
/**
 * Strip TypeScript-specific syntax so acorn can parse as JS.
 */
...
/**
 * Parse a JS/TS file with acorn and extract all identifiers.
 */
export function parseJsFile(code, filePath) {
  const identifiers = [];
...
/**
 * Parse a Python file using python3's ast module.
 */
export function parsePythonFile(filePath) {
...
/**
 * Scan project files for usage of specific affected API names.
 * Used by the migration feature.
 */
...
/**
 * Scan project and return ALL identifiers (not filtered by specific APIs).
 * Used by the review feature to find all API usages.
 */
export async function scanProjectAllApis(projectDir, lang = "js") {
  const extensions = LANG_EXTENSIONS[lang] || LANG_EXTENSIONS.js;
  const files = await getFiles(projectDir, extensions);
  const results = [];
