...
DocAware is available as open-source software at \url{https://github.com/[redacted]/docaware-ai}.
...
The review pipeline uses Claude (Anthropic) as the underlying LLM, invoked through a structured tool-use interface rather than free-text generation. This design choice ensures:
...
\begin{enumerate}
    ...
    \item \textbf{Tool-use interfaces improve reliability.} Using Claude's tool-use feature for structured JSON output eliminates parsing failures that plague free-text extraction.
\end{enumerate}
...
\textbf{Future work.} We plan to extend DocAware to additional languages (Python, Go, Rust), evaluate on larger real-world codebases, develop more nuanced hallucination metrics, and conduct longitudinal studies to measure the memory layer's cumulative benefits.
